...
// ---------------------------------------------------------------------------
const { mockRunGenerateWorkflow } = vi.hoisted(() => ({
  mockRunGenerateWorkflow: vi.fn().mockResolvedValue({ appUrl: 'https://app.example.com' }),
}))

vi.mock('../../src/workflows/generate', () => ({
  runGenerateWorkflow: mockRunGenerateWorkflow,
}))
...
describe('generateRoute', () => {
  ...
  it('responds with 202 and a confirmation message, then calls runGenerateWorkflow', async () => {
    ...
    expect(mockRunGenerateWorkflow).toHaveBeenCalledWith({
      pageId,
      userId,
  ...
  it('returns 400 when pageId is missing', async () => {
    ...
    expect(mockRunGenerateWorkflow).not.toHaveBeenCalled()
  })
  ...
  it('does not throw synchronously when runGenerateWorkflow rejects', async () => {
    mockRunGenerateWorkflow.mockRejectedValueOnce(new Error('workflow failed'))

    const req = mockReq({ data: { id: pageId }, source: { user_id: userId } })
    const res = mockRes()

    // Should not throw — the .catch in the handler absorbs the rejection
    await expect(generateRoute(req, res)).resolves.toBeUndefined()

    expect(res.status).toHaveBeenCalledWith(202)
    expect(mockRunGenerateWorkflow).toHaveBeenCalled()
  })
})
